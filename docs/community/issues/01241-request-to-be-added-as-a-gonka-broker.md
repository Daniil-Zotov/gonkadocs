---
title: "#1241 — Request to be added as a Gonka broker"
source: https://github.com/gonka-ai/gonka/issues/1241
issue_number: 1241
synced_at: 2026-07-09T18:14:32Z
template: issues-main.html
---

<div class="issues-detail-header">
  <h1 class="issues-detail-title">
    <span class="issues-status issues-status-closed"><svg viewBox="0 0 16 16"><path d="M13.78 4.22a.75.75 0 0 1 0 1.06l-7.25 7.25a.75.75 0 0 1-1.06 0L2.22 9.28a.751.751 0 0 1 .018-1.042.751.751 0 0 1 1.042-.018L6 10.94l6.72-6.72a.75.75 0 0 1 1.06 0Z"/></svg></span>
    Request to be added as a Gonka broker
    <span class="issues-number">#1241</span>
  </h1>
  <div class="issues-detail-meta">
    <span class="issues-meta-item">Closed</span>
    <span class="issues-meta-item">[@olkwwuah](https://github.com/olkwwuah) opened 2026-05-24 17:24 UTC</span>
    <span class="issues-meta-item">0 comments</span>
    <span class="issues-meta-item">Updated 2026-05-24 17:26 UTC</span>
  </div>
  <div class="issues-labels" style="margin-top: 8px;"></div>
</div>

<div class="issues-content" markdown="1">
Hi Gonka core team & community,

I'm requesting inclusion as a Gonka broker and inclusion of my address in the devshard creator allow-list.

Operator name: Daniel / LabdaLab
Contact: Discord: @labdalab, Telegram: @That_metalhead

URL: https://gonkadalab.com

Devshard address:
gonka15uuzwv36ln8mlsmu7ccg6rr3ntj9mh7t9x6n8u

Models:
Qwen/Qwen3-235B-A22B-Instruct-2507
moonshotai/Kimi-K2.6

Rate limits:
60 RPM API key on the starter plan

Billing:
Crypto, GNK

Target audience:
Developers who need an OpenAI-compatible Gonka API endpoint.

Thanks, and I wish everyone a great day!
</div>

---

> 🔄 **Auto-synced** from [Issue #1241](https://github.com/gonka-ai/gonka/issues/1241) every hour.
