---
title: "#654 — Optimize mlnode: reduce mlnode image size, refactor api service (proxy part for the start)"
source: https://github.com/gonka-ai/gonka/issues/654
issue_number: 654
synced_at: 2026-07-06T09:52:34Z
---

> 🔄 **Авто-синхронизация:** из [Issue #654](https://github.com/gonka-ai/gonka/issues/654) каждые 6 часов. 

# 🟢 Optimize mlnode: reduce mlnode image size, refactor api service (proxy part for the start)

**Автор:** [@tcharchian](https://github.com/tcharchian) · **Состояние:** Open · **Создано:** 2026-01-27 19:11 UTC · **Обновлено:** 2026-03-21 23:33 UTC

---

## 📝 Описание

*(пусто)*
