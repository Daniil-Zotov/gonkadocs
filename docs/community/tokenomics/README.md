...
Mirrored from [gonka-code/docs/tokenomics.md](/gonka-code/docs/tokenomics.md) (source: [gonka-ai/gonka](https://github.com/gonka-ai/gonka)).
...
1.  **Initial Escrow**: Before any on-chain transaction, the `decentralized-api` first verifies that the user has sufficient funds to cover the maximum possible cost of the request. This check can be seen in [`decentralized-api/internal/server/public/post_chat_handler.go`](/gonka-code/decentralized-api/internal/server/public/post_chat_handler.go). If the check passes, an amount is placed in escrow on the blockchain to guarantee payment to the provider. The on-chain escrow calculation can be found in `CalculateEscrow` in [`inference-chain/x/inference/calculations/inference_state.go`](/gonka-code/inference-chain/x/inference/calculations/inference_state.go).
...
Unlike many other blockchains, this network does not charge separate fees (i.e., "gas") for submitting transactions. The configuration, as seen in [`decentralized-api/cosmosclient/cosmosclient.go`](/gonka-code/decentralized-api/cosmosclient/cosmosclient.go), sets transaction fees to zero.
...
The process of claiming rewards and the distribution of work and reward coins are handled in [`inference-chain/x/inference/keeper/msg_server_claim_rewards.go`](/gonka-code/inference-chain/x/inference/keeper/msg_server_claim_rewards.go). The minting of new reward coins is done by the `MintRewardCoins` function in [`inference-chain/x/inference/keeper/payment_handler.go`](/gonka-code/inference-chain/x/inference/keeper/payment_handler.go).
...
This system is designed to incentivize participation, especially in the early stages of the network, and it gradually reduces the number of new coins over time. The specific logic for this calculation can be found in [`inference-chain/x/inference/keeper/accountsettle.go`](/gonka-code/inference-chain/x/inference/keeper/accountsettle.go).
...
*For detailed technical specifications, see the [Collateral Proposal](https://github.com/gonka-ai/gonka/blob/main/docs/proposals/tokenomics-v2/collateral.md).*
...
*For detailed technical specifications, see the [Vesting Proposal](https://github.com/gonka-ai/gonka/blob/main/docs/proposals/tokenomics-v2/vesting.md).*

## Token Supply

The initial total supply of the native coin and its distribution are defined in the `DefaultGenesisOnlyParams` function in [`inference-chain/x/inference/types/params.go`](/gonka-code/inference-chain/x/inference/types/params.go). This includes the allocation for the originator, top reward amount, pre-programmed sale amount, and standard reward amount.